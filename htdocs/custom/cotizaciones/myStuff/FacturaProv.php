...
for ($i=0; $i < $LINEAS; $i++) { 
	...
	if ($Asign != null) {
		...
		*/
		...
		$data["fk_propal"] = "75" ;
		//$Pago = $object->$Pago;
		$data["multicurrency_subprice"] = $Pago;
		$data["product_type"] = "0" ;
		$data["qty"] = "1" ;
		$data["specimen"] = 0 ;
		$data["subprice"]= "1333.00";//$object->$Pago;
	...
	if (isSet($AgregarLineas) & $AgregarLineas != null) {
		...
		print dolGetButtonAction($langs->trans('ir a Facturas de provedores'), '', 'default', $Rooo.'fourn/facture/list.php?leftmenu=suppliers_bills', '', $permissiontoadd);
		
	}
echo '<br> FIN 0 '.$Message;
