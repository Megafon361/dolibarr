...
/**
 *  \file       cotizacion_contact.php
 *  \ingroup    cotizaciones
 *  \brief      Tab for contacts linked to Cotizacion
 */
...
$hookmanager->initHooks(array('cotizacioncontact', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permission = $user->rights->cotizaciones->cotizacion->write;
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
$title = $langs->trans('Cotizacion')." - ".$langs->trans('ContactsAddresses');
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'contact', $langs->trans("Cotizacion"), -1, $object->picto);
