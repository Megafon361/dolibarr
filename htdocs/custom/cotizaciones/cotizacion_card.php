...
// });
// </script>';
print '<link rel="stylesheet" type="text/css" href="semantic/dist/semantic.min.css">
<script
...
print '<select class="ui search dropdown">
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("Cotizacion"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	$head = cotizacionPrepareHead($object);
	print dol_get_fiche_head($head, 'card', $langs->trans("Cotizacion"), -1, $object->picto);

	$formconfirm = '';

	// Confirmation to delete
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteCotizacion'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 1);
	}
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			$filedir = $conf->cotizaciones->dir_output.'/'.$object->element.'/'.$objref;
			$urlsource = $_SERVER["PHP_SELF"]."?id=".$object->id;
			$genallowed = $permissiontoread; // If you can read, you can build the PDF to read content
			$delallowed = $permissiontoadd; // If you can create/edit, you can remove a file on card
			print $formfile->showdocuments('cotizaciones:Cotizacion', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('cotizacion'));
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-list-alt imgforviewmode', dol_buildpath('/cotizaciones/cotizacion_agenda.php', 1).'?id='.$object->id);
	...
	$diroutput = $conf->cotizaciones->dir_output;
	$trackid = 'cotizacion'.$object->id;
