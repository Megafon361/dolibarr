...
/**
 *  \file       cotizacion_agenda.php
 *  \ingroup    cotizaciones
 *  \brief      Tab of events on Cotizacion
 */
...
$hookmanager->initHooks(array('cotizacionagenda', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->cotizaciones->multidir_output[!empty($object->entity) ? $object->entity : $conf->entity]."/".$object->id;
}
...
if ($object->id > 0) {
	...
	print dol_get_fiche_head($head, 'agenda', $langs->trans("Cotizacion"), -1, $object->picto);
	...
	if (!empty($conf->agenda->enabled) && (!empty($user->rights->agenda->myactions->read) || !empty($user->rights->agenda->allactions->read))) {
		...
		//print load_fiche_titre($langs->trans("ActionsOnCotizacion"), '', '');
